...
def test_learn_review_again_schedules_2_minutes_and_then_becomes_due(monkeypatch, client):
    from app.routers import learn as learn_router
    ...
    # Review at t=00:00:00Z
    monkeypatch.setattr(learn_router, "utc_now_iso", lambda: "2025-12-13T00:00:00Z")

    review = client.post(
        "/learn/review",
        headers={"X-User-Id": user_id},
        json={"deckId": deck_id, "cardId": card_id, "grade": "again"},
    )
    assert review.status_code == 200
    updated = review.json()
    assert updated["dueAt"] == "2025-12-13T00:02:00Z"
    assert updated["lastReviewedAt"] == "2025-12-13T00:00:00Z"

    # Immediately after review: no card due
    nxt = client.get(f"/learn/next?deckId={deck_id}", headers={"X-User-Id": user_id})
    assert nxt.status_code == 200
    data = nxt.json()
    assert data["card"] is None
    assert data["nextDueAt"] == "2025-12-13T00:02:00Z"

    # After 2 minutes: card is due again
    monkeypatch.setattr(learn_router, "utc_now_iso", lambda: "2025-12-13T00:02:00Z")
    nxt2 = client.get(f"/learn/next?deckId={deck_id}", headers={"X-User-Id": user_id})
    assert nxt2.status_code == 200
    data2 = nxt2.json()
    assert data2["card"]["id"] == card_id
