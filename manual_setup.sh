...
#   - Azure CLI (az)
#   - Run ./setup_local_auth.sh to create app registrations
#
# Usage:
#   ./manual_setup.sh           # Start with auth disabled (default)
#   ./manual_setup.sh --auth    # Start with auth enabled (requires setup_local_auth.sh first)
...
echo -e "${BLUE}"
echo "╔═══════════════════════════════════════════╗"
echo "║       Echo App - Local Development        ║"
echo "╚═══════════════════════════════════════════╝"
echo -e "${NC}"
...
if [ "${1:-}" == "--auth" ]; then
    ...
    if grep -q "^VITE_AZURE_CLIENT_ID=.\+" "$SCRIPT_DIR/frontend/.env.local" 2>/dev/null; then
        echo "✓ Auth configuration found"
        
        # Enable auth in both files
        sed -i.bak 's/^AUTH_ENABLED=.*/AUTH_ENABLED=true/' "$SCRIPT_DIR/backend/.env"
        sed -i.bak 's/^VITE_AUTH_ENABLED=.*/VITE_AUTH_ENABLED=true/' "$SCRIPT_DIR/frontend/.env.local"
        rm -f "$SCRIPT_DIR/backend/.env.bak" "$SCRIPT_DIR/frontend/.env.local.bak"
        
        echo "✓ Auth enabled in environment files"
    else
        echo ""
        echo "❌ Auth is not configured. Please run first:"
        echo ""
        echo "   ./setup_local_auth.sh"
        echo ""
        echo "Then try again with: ./manual_setup.sh --auth"
        exit 1
    fi
fi
...
COSMOS_EMULATOR=$(grep "^COSMOS_EMULATOR=" "$SCRIPT_DIR/backend/.env" | cut -d'=' -f2)
if [ "$COSMOS_EMULATOR" == "true" ]; then
    # Check if emulator is running
    if ! curl -k -s https://localhost:8081/_explorer/emulator.pem > /dev/null 2>&1; then
        echo ""
        echo -e "${YELLOW}⚠ Cosmos DB Emulator is not running${NC}"
        echo ""
...
AUTH_ENABLED=$(grep "^AUTH_ENABLED=" "$SCRIPT_DIR/backend/.env" | cut -d'=' -f2)
if [ "$AUTH_ENABLED" == "true" ]; then
    echo "  🔐 Authentication: ENABLED (Entra ID)"
else
    echo "  🔓 Authentication: DISABLED (dev mode)"
    echo ""
    echo "  To enable Entra auth, run: ./setup_local_auth.sh"
fi
